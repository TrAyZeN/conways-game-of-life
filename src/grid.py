...
class Grid:
    ...
    def update(self):
        _grid_next_generation = np.zeros(
            (self.width, self.height), dtype=np.int8)
        for y in range(self.height):
            for x in range(self.width):
                _grid_next_generation[y][x] = self._should_survive(
                    (x, y), self._get_neighbours_number((x, y)))
    ...
    def _should_survive(self, cell_coords, neighbours_number):
        return (neighbours_number == 3) \
            or (self.is_alive(cell_coords) and neighbours_number == 2)
    ...
    def invert_cell_state(self, cell_coords):
        self._grid[cell_coords[1]][cell_coords[0]] = \
            int(not self._grid[cell_coords[1]][cell_coords[0]])
